 ...
 * Will not allow blank field or non numeric value. Only creates an event on
 * actual change.
 */
public class ArmorField extends JPanel implements FocusListener {
	...
	// Hold which has focus and it's starting value to compare when it loses
	// focus.
	 ...
	 *
	 * @param target
	 *            The ArmorForm this field is managing
	 */
	public ArmorField(com.ageoflegacy.aedit.ui.view.mobView.ArmorForm target) {
		this.target = target;
	...
	/**
	 * Called when one of the JTextFields loses focus, reverts change if box is
	 * blanked out or changed to a non-number. Discards the event if the value
	 * did not change.
	 *
	 * @param event
	 *            FocusEvent
